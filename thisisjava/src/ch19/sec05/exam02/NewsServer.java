			...
			public void run() {
				try {
					...
					while(true) {
						 ...
						 * 
						 * //클라이언트의 IP와 port얻기 SocketAddress socketAddress =
						 * receivePacket.getSocketAddress();
						 * 
						 * //10개의 뉴스를 클라이언트로 전송 for(int i=1; i<=10; i++) { String data = newsKind +
						 * ": 뉴스" + i; byte[] bytes = data.getBytes("UTF-8"); DatagramPacket sendPacket
						 * = new DatagramPacket(bytes,0,bytes.length,socketAddress);
						 * datagramSocket.send(sendPacket); }
						 */
					}
